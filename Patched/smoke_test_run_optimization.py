...
def check_C():
    ...
    assert feas["counts"]["train"] == 23 * 4, feas["counts"]
    # val segment 120 s = 6000 samples, stride 1500 -> floor((6000-1500)/1500)+1 = 4
    assert feas["counts"]["val"] == 4 * 4, feas["counts"]
    ok("C", "default (200 s) correctly REJECTED with a fix-naming message; "
            "feasible geometry passes and window counts match a hand calculation")
...
def check_D():
    ...
    # both block families reported at each corner  [ADDED 5]
    assert any(k.endswith("blk0") for k in sizes["corners"])
    assert any(k.endswith("blk1") for k in sizes["corners"])
    ...
    ok("D", "meta counts == real counts; corner table reproduces the docs "
            "(300192 / 2450144 / 17734112 / 214441248); OOM warning fires")
...
def check_F():
    ...
    # a malformed schema must name the missing key
    with open(specs_path, "w", encoding="ascii") as fh:
        json.dump([{"name": "x", "condition": 0}], fh)      # no "path"
    cfg3 = ExperimentConfig.from_dict(cd)
    cfg3.runtime = replace(cfg3.runtime, cache_dir=str(d / "cache3"))
    try:
        build_traces(cfg3)
        raise AssertionError("a missing 'path' key must raise")
    except ValueError as ex:
        assert "path" in str(ex)
    ok("F", "numpy: specs-relative paths resolved; missing file and malformed "
            "schema both raise with the offending item named")
